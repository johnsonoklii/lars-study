...
, m_socket(csocket::create_socket(AF_INET, SOCK_DGRAM | SOCK_NONBLOCK | SOCK_CLOEXEC, IPPROTO_UDP)) {
    ...
    m_server_addrlen = sizeof(m_server_addr);
    ...
    //2 链接
    int ret = connect(m_socket.fd(), (const struct sockaddr*)&m_server_addr, m_server_addrlen);
    if (ret  == -1) {
        perror("connect");
        exit(1);
    }

    m_loop->add_io_event(m_socket.fd(), EPOLLIN, std::bind(&udp_client::do_read, this));
}
...
void udp_client::do_read() {
    ...
    while (true) {
        int pkg_len = recvfrom(m_socket.fd(), m_read_buf, sizeof(m_read_buf), 0, (struct sockaddr *)&m_server_addr, &m_server_addrlen);
        if (pkg_len == -1) {
            if (errno == EINTR) {
                continue;
            }
...
int udp_client::send(int msgid, const char* data, int len) {
    ...
    // 3. 发送
    int ret = sendto(m_socket.fd(), m_write_buf, head.len, 0, (struct sockaddr*)&m_server_addr, m_server_addrlen);
    if (ret < 0) {
        perror("sendto...");
        return -1;
    }
