#pragma once

#include "handler.h"
#include "event_loop.h"
#include "net_connection.h"
#include "socket.h"

#include <memory>

class udp_client: public net_connection, public std::enable_shared_from_this<udp_client> {
public:
    using sptr = std::shared_ptr<udp_client>;
    udp_client(event_loop* loop, const char* ip, uint16_t port);
    ~udp_client();

    void add_router(int msgid, handler_t handler) { handler_map::get_instance().register_handler(msgid, handler); }

    int fd() override { return m_socket.fd(); }
    int send(int msgid, const char* data, int len) override;

private:
    void do_read();

private: 
    event_loop* m_loop;
    csocket m_socket;

    char m_read_buf[1024];
    char m_write_buf[1024];

    sockaddr_in m_server_addr;
    socklen_t m_server_addrlen;
};